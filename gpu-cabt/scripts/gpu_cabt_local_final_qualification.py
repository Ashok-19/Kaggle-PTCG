...
from gpu_cabt.device_runtime import GpuCabtRuntime, default_official_dir, repo_root
from gpu_cabt.nvrtc import load_cupy_module
from gpu_cabt.rule_static import extract_rule_tables

LUCARIO = Path("data/decks/lucario-modern-v1.csv")
OPPONENTS = (
    ("dragapult", Path("data/decks/dragapult-ex.csv")),
    ("alakazam", Path("data/decks/alakazam-v9.csv")),
    ("lopunny", Path("data/decks/lopunny-v9.csv")),
    ("iono", Path("data/decks/iono.csv")),
    ("abomasnow", Path("data/decks/mega-abomasnow-ex.csv")),
    ("grim", Path("data/decks/grim-v15-control.csv")),
)
...
def native_benchmark(root: Path, decks: list[np.ndarray], repeats: int) -> dict[str, Any]:
    official = default_official_dir(); source = root / "scripts/gpu_cabt_native_final_benchmark.cpp"; payload = " ".join(str(int(c)) for deck in decks for c in deck) + "\n"
...
def main() -> None:
    ap = argparse.ArgumentParser(); ap.add_argument("--samples", type=int, default=1); ap.add_argument("--native-repeats", type=int, default=500); ap.add_argument("--stress-envs", type=int, default=6000); ap.add_argument("--stack-envs", type=int, default=768); ap.add_argument("--check-interval", type=int, default=16); ap.add_argument("--seed", type=int, default=20260814); ap.add_argument("--env-count", action="append", type=int); ap.add_argument("--cpu-core-limit", type=int, default=2); ap.add_argument("--max-envs", type=int, default=12000); ap.add_argument("--output", type=Path, default=Path("reports/local/gpu-cabt-standalone-qualification.json")); args = ap.parse_args()
