"""Public-only ceiling probe for the immediate opponent choice sidecar.

The sidecar is label-audit data.  Only the complete-root dataset supplies
features: the frozen public G2 hidden state and the candidate root option
embedding.  No opponent request, legal set, private state, identity,
determinization, or post-choice state crosses that boundary.
"""
...
class Candidate:
    ...
    def stratum(self) -> tuple[str, str]:
        return ("::".join(self.anchor_pair), self.seat)
...
def _int(value: Any, label: str, *, minimum: int = 0, maximum: int | None = None) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value < minimum:
        raise InterchangeError(f"{label} is not a valid integer")
    if maximum is not None and value > maximum:
        raise InterchangeError(f"{label} is outside the semantic contract")
    return value
...
def _walk_firewall(value: Any, path: str = "public_tensor") -> None:
    forbidden = {
        ...
        "terminal_engine_result", "legal_options", "legal_set", "post_choice_state",
        "future_information",
    }
    if isinstance(value, Mapping):
        ...
        for index, child in enumerate(value):
            _walk_firewall(child, f"{path}[{index}]")
...
def _feature(tensor: Mapping[str, Any], option_index: int, trunk: Any | None = None) -> tuple[float, ...]:
    ...
    if len(values) != FEATURE_WIDTH or not all(math.isfinite(float(value)) for value in values):
...
def _call_extractor(
    ...
    if len(values) != FEATURE_WIDTH or not all(math.isfinite(value) for value in values):
...
def _validate_label_schema(sidecar: Mapping[str, Any]) -> None:
    ...
    if jsonschema is not None:
        ...
        script = (
            "const fs=require('fs');const Ajv2020=require('ajv/dist/2020');"
            "const s=JSON.parse(fs.readFileSync(process.argv[1]));const d=JSON.parse(fs.readFileSync(0));"
            "const v=new Ajv2020({allErrors:true,strict:false}).compile(s);"
            "process.stdout.write(JSON.stringify(v(d)?[]:v.errors));"
        )
        result = subprocess.run(
            ["node", "-e", script, str(LABEL_SCHEMA)], input=json.dumps(value),
...
def _root_index(root_path: Path) -> tuple[dict[str, Mapping[str, Any]], str]:
    ...
    groups, run = document.get("state_groups"), document.get("run")
    ...
    for group in groups:
        ...
        if (
            ...
            or len(replicates) != PARTICLES
            or {rep.get("replicate_id") for rep in replicates if isinstance(rep, Mapping)} != set(range(PARTICLES))
        ):
        ...
        expected = [option.get("original_index", index) for index, option in enumerate(options) if isinstance(option, Mapping)]
        if (
            not isinstance(original_indices, list) or len(original_indices) != len(options)
            or len(set(original_indices)) != len(options) or sorted(original_indices) != sorted(expected)
        ):
        ...
        action_sets = [
            {action.get("action_id") for action in rep.get("actions", []) if isinstance(action, Mapping)}
            for rep in replicates
        ]
        if (
            any(not isinstance(value, str) or not value for value in determinization_ids)
            or len(set(determinization_ids)) != PARTICLES
            or any(not values for values in action_sets)
            or len({frozenset(values) for values in action_sets}) != 1
        ):
...
def _projection_bindings(sidecar: Mapping[str, Any], roots: Mapping[str, Mapping[str, Any]]) -> dict[str, Mapping[str, Any]]:
    ...
    for row in rows:
        ...
        if row.get("history_tokens_sha256") != _stable_hash(root["public_tensor"]["history_tokens"]):
...
def _request_shape(request: Mapping[str, Any]) -> tuple[int, int, int, int, str, list[Mapping[str, Any]]]:
    selection_type = _int(request.get("selection_type"), "selection_type")
    selection_context = _int(request.get("selection_context"), "selection_context")
    minimum = _int(request.get("min_count"), "min_count")
    maximum = _int(request.get("max_count"), "max_count")
    ordering = request.get("ordering")
    if ordering not in {"ORDERED", "UNORDERED"}:
        raise InterchangeError("request ordering is malformed")
    expected = "ORDERED" if (selection_type, selection_context) == (5, 34) else "UNORDERED"
    if ordering != expected:
        raise InterchangeError("selection type/context ordering is inconsistent")
    options = request.get("options")
    option_count = _int(request.get("option_count"), "option_count", minimum=1)
    if not isinstance(options, list) or option_count != len(options):
        raise InterchangeError("opponent request does not retain the complete legal option set")
    if maximum < minimum or maximum > option_count:
        raise InterchangeError("request selection bounds are malformed")
    indices: list[int] = []
    for option in options:
        if not isinstance(option, Mapping):
            raise InterchangeError("opponent legal option is malformed")
        index = _int(option.get("original_index"), "opponent option original_index")
        if not _hash(option.get("semantic_equivalence_key")) or not _hash(option.get("semantic_fingerprint")):
            raise InterchangeError("opponent option semantic identity is malformed")
        if option.get("is_stop") is not False:
            raise InterchangeError("opponent option STOP state is not supported by this sidecar")
        indices.append(index)
    if len(set(indices)) != len(indices) or sorted(indices) != list(range(option_count)):
        raise InterchangeError("opponent legal option indexes are not complete and contiguous")
    return selection_type, selection_context, minimum, maximum, ordering, options

def _collector_action_key(ordering: str, keys: Sequence[str]) -> str:
    if len(keys) == 1:
        return keys[0]
    canonical = list(keys) if ordering == "ORDERED" else sorted(keys)
    return _stable_hash({"ordering": ordering, "semantic_path": canonical})

def _context_key(selection_type: int, selection_context: int) -> str:
    return f"{selection_type}:{selection_context}"

def _target_key(selection_type: int, selection_context: int, ordering: str, keys: Sequence[str]) -> str:
    canonical = list(keys) if ordering == "ORDERED" else sorted(keys)
    return _stable_hash({
        "selection_type": selection_type,
        "selection_context": selection_context,
        "ordering": ordering,
        "semantic_action_path": canonical,
    })

def _observed_target(label: Mapping[str, Any]) -> tuple[str, str, int]:
    ...
    selection_type, selection_context, minimum, maximum, ordering, options = _request_shape(request)
    ...
    if not isinstance(indices, list) or not indices or len(indices) != len(set(indices)):
        raise InterchangeError("chosen transport path is malformed or repeats an option")
    if not isinstance(path, list) or len(path) != len(indices) or not all(isinstance(item, Mapping) for item in path):
        raise InterchangeError("chosen semantic path is malformed")
    if not minimum <= len(path) <= maximum:
        raise InterchangeError("chosen semantic path violates request selection bounds")
    by_original = {int(option["original_index"]): option for option in options}
    keys: list[str] = []
    for index, path_option in zip(indices, path):
        selected = by_original.get(_int(index, "chosen transport index"))
        if selected is None or _canonical(path_option) != _canonical(selected):
        ...
        if path_option.get("semantic_fingerprint") != selected.get("semantic_fingerprint"):
            raise InterchangeError("chosen audit fingerprint differs from selected option")
        keys.append(str(selected["semantic_equivalence_key"]))
    expected_action_key = _collector_action_key(ordering, keys)
    if chosen.get("semantic_equivalence_key") != expected_action_key:
        raise InterchangeError("chosen action equivalence key is inconsistent with its path")
    if chosen.get("semantic_action_fingerprint") != _stable_hash(path):
        raise InterchangeError("chosen action fingerprint does not bind its path")
    return (
        _target_key(selection_type, selection_context, ordering, keys),
        _context_key(selection_type, selection_context),
        len(options),
    )

def _validate_sidecar(
    sidecar_path: Path, sidecar: Mapping[str, Any], root_path: Path,
) -> tuple[dict[str, Mapping[str, Any]], str, dict[str, Mapping[str, Any]], dict[tuple[str, str], str]]:
    ...
    binding = sidecar.get("dataset_binding")
    ...
    if binding.get("dataset_sha256") != _sha256(root_path):
    ...
    if not isinstance(binding.get("state_group_ids"), list) or set(binding["state_group_ids"]) != set(roots):
    ...
    if sidecar.get("firewall") != expected_firewall:
    ...
    raw_keys = sidecar.get("root_action_keys")
    if not isinstance(raw_keys, list) or not raw_keys:
        raise InterchangeError("root_action_keys is empty")
    for row in raw_keys:
        if not isinstance(row, Mapping) or not isinstance(row.get("state_group_id"), str) or not isinstance(row.get("action_id"), str) or not _hash(row.get("semantic_equivalence_key")):
    ...
    dataset_actions = {
        (state, action.get("action_id"))
        for state, root in roots.items()
        for replicate in root.get("replicates", []) if isinstance(replicate, Mapping)
        for action in replicate.get("actions", []) if isinstance(action, Mapping)
    }
...
def load_branch_sidecar(
    sidecar_path: str | Path, root_dataset_path: str | Path, *, trunk: Any | None = None,
    feature_extractor: Callable[..., Sequence[float]] | None = None, trusted_trunk: bool = False,
) -> tuple[list[Candidate], dict[str, Any]]:
    """Load observed immediate-choice labels and bind them to root features."""
    ...
    grouped: dict[tuple[str, str], dict[int, tuple[str, str, int, tuple[float, ...], str]]] = defaultdict(dict)
    particle_ids: dict[tuple[str, str, int], str] = {}
    errors: list[str] = []
    supported = 0
    policy = str(sidecar["provenance"].get("opponent_policy_id", "UNKNOWN"))
    baseline = str(sidecar["provenance"].get("anchor_baseline_id", "UNKNOWN"))
    ...
    for index, label in enumerate(labels):
        try:
            ...
            if status != "OBSERVED":
                request = label.get("first_opponent_request")
                chosen = label.get("chosen_action")
                if status == "UNSUPPORTED_FIRST_OPPONENT_REQUEST":
                    if not isinstance(request, Mapping) or chosen is not None:
                        raise InterchangeError("unsupported label must retain request and no chosen action")
                    _request_shape(request)
                elif status in {"TERMINAL_BEFORE_OPPONENT", "MISSING_OR_ERROR"} and chosen is not None:
                    raise InterchangeError("non-observed label carries a chosen action")
                continue
            if label.get("error") is not None:
                raise InterchangeError("observed label carries an error")
            state, replicate, action_id = label.get("state_group_id"), label.get("replicate_id"), label.get("action_id")
            ...
            root_key = action_keys.get((str(state), action_id))
            if root_key != label.get("root_action_semantic_equivalence_key"):
                raise InterchangeError("label root action equivalence differs from root_action_keys")
            target, target_context, legal_count = _observed_target(label)
            ...
            feature = _call_extractor(extract, root["public_tensor"], _learner_index(root, option_index), trunk)
            particle = label.get("particle_id")
            particle_key = (str(state), str(root_key), replicate)
            ...
            group = (str(state), str(root_key))
            row = (target, target_context, legal_count, feature, action_id)
            previous = grouped[group].get(replicate)
            if previous is not None and previous[:4] != row[:4]:
    ...
    incomplete: list[str] = []
    expected_groups = {(state, key) for (state, _action), key in action_keys.items()}
    for group in sorted(expected_groups):
        if group not in grouped or set(grouped[group]) != set(range(PARTICLES)):
            incomplete.append(f"{group[0]}:{group[1]}")
    for (state, root_key), rows in sorted(grouped.items()):
        if set(rows) != set(range(PARTICLES)):
            continue
        ordered = [rows[particle] for particle in range(PARTICLES)]
        if len({item[3] for item in ordered}) != 1:
            errors.append(f"{state}:{root_key}:feature drift across particles")
            continue
        root = roots[state]
        anchor_pair = (baseline, policy)
        candidates.append(Candidate(
            state, root_key, baseline, f"P{root.get('root_player', root.get('acting_player', 'UNKNOWN'))}",
            anchor_pair, ordered[0][3], tuple(item[0] for item in ordered),
            tuple(item[1] for item in ordered), tuple(item[2] for item in ordered),
        ))
    ingest = {
        "raw_label_count": len(labels), "supported_label_count": supported,
        "supported_group_count": len(candidates), "unsupported_label_count": len(labels) - supported,
        "join_errors": errors, "firewall_errors": [error for error in errors if "firewall" in error.lower() or "private" in error.lower() or "opponent" in error.lower()],
        "reliability_errors": [error for error in errors if "drift" in error.lower() or "alias" in error.lower()],
        "incomplete_groups": sorted(set(incomplete)), "declared_group_count": len(expected_groups),
        "dataset_sha256": _sha256(root_file), "run_id": run_id, "root_ids": sorted(roots),
        "config_sha256": sidecar["run"].get("config_sha256"), "profile": sidecar["run"].get("profile"),
        "custom_feature_extractor": feature_extractor is not None or (trunk is not None and not trusted_trunk),
        "source_commit": sidecar["run"].get("source_commit"), "anchor_policy": anchor_pair,
    }
    ingest["status_counts"] = dict(status_counts)
    return candidates, ingest

def stratified_group_split(records: Sequence[Candidate]) -> dict[str, list[Candidate]]:
    """Split whole roots, retaining anchor-pair and seat strata."""
    ...
    for row in records:
        if row.root in strata and strata[row.root] != row.stratum:
            raise ValueError(f"root crosses anchor/seat strata: {row.root}")
    ...
    for stratum in sorted(set(strata.values())):
        roots = sorted((root for root, value in strata.items() if value == stratum), key=lambda value: hashlib.sha256(value.encode()).hexdigest())
        train_count = min(len(roots), max(1 if roots else 0, round(len(roots) * 0.625)))
        tune_count = min(len(roots) - train_count, round(len(roots) * 0.1875))
        for root, split in zip(roots, ["train"] * train_count + ["tune"] * tune_count + ["test"] * (len(roots) - train_count - tune_count)):
...
def _fit(train: Sequence[Candidate]) -> tuple[LinearProbe | None, tuple[str, ...], dict[str, str], dict[str, Any]]:
    class_contexts: dict[str, str] = {}
    for row in train:
        for target, context in zip(row.targets, row.target_contexts):
            previous = class_contexts.setdefault(target, context)
            if previous != context:
                raise FirewallError("context-bound target key maps to multiple request contexts")
    classes = tuple(sorted(class_contexts))
    if not train or not classes:
        return None, classes, class_contexts, {"status": "BLOCKED_NO_TRAIN_LABELS"}
    torch.manual_seed(17)
    model = LinearProbe(len(classes))
    values = torch.tensor([row.feature for row in train], dtype=torch.float32).repeat_interleave(PARTICLES, dim=0)
    targets = torch.tensor([classes.index(target) for row in train for target in row.targets], dtype=torch.long)
    optimizer = torch.optim.SGD(model.parameters(), lr=0.08)
    for _ in range(240):
        optimizer.zero_grad(set_to_none=True)
        loss = nn.functional.cross_entropy(model(values), targets)
        if not torch.isfinite(loss):
            raise FirewallError("linear probe loss became nonfinite")
        loss.backward()
        optimizer.step()
    return model.eval(), classes, class_contexts, {"status": "FIT", "seed": 17, "steps": 240, "learning_rate": 0.08, "class_count": len(classes), "train_roots": len({row.root for row in train})}
...
def _ceiling(rows: Sequence[Candidate]) -> dict[str, Any]:
    ...
    roots = [sum(scores) / len(scores) for scores in by_root.values()]
    rng = random.Random(23)
    samples = sorted(sum(rng.choice(roots) for _ in roots) / len(roots) for _ in range(4000)) if roots else []
    return {"top1": sum(value[0] for value in values) / len(values) if values else None, "top3": sum(value[1] for value in values) / len(values) if values else None, "root_bootstrap_top3_lcb95": samples[99] if len(samples) >= 100 else None}

def _report(
    rows: Sequence[Candidate], train: Sequence[Candidate], model: LinearProbe | None,
    classes: tuple[str, ...], class_contexts: Mapping[str, str],
) -> dict[str, Any]:
    total = correct1 = correct3 = unseen = context_total = context_correct = context_unseen = 0
    nll = 0.0
    ...
    context_classes = sorted(set(class_contexts.values()))
    for row_index, row in enumerate(rows):
        ranked = torch.argsort(probabilities[row_index], descending=True).tolist() if model else []
        context_probabilities = {context: 0.0 for context in context_classes}
        for class_index, target in enumerate(classes):
            context_probabilities[class_contexts[target]] += float(probabilities[row_index, class_index].detach())
        predicted_context = max(context_classes, key=lambda context: (context_probabilities[context], context), default=None)
        for target, actual_context in zip(row.targets, row.target_contexts):
            context_total += 1
            if actual_context not in context_probabilities:
                context_unseen += 1
            ...
            nll -= math.log(max(float(probabilities[row_index, class_index]), 1e-12))
            correct1 += bool(ranked and ranked[0] == class_index)
            correct3 += bool(ranked and class_index in ranked[:3])
            top3_by_root[row.root].append(float(bool(ranked and class_index in ranked[:3])))
    ...
    return {
        ...
        "context_top1": context_correct / context_total if context_total else None,
        "context_unseen_rate": context_unseen / context_total if context_total else None,
        "legal_uniform_audit": {
            "top1": sum(1 / count for count in uniform_rows) / len(uniform_rows) if uniform_rows else None,
            "top3": sum(min(3, count) / count for count in uniform_rows) / len(uniform_rows) if uniform_rows else None,
            "nll": sum(math.log(count) for count in uniform_rows) / len(uniform_rows) if uniform_rows else None,
            "feature_use": "audit-only",
        },
    }
...
def _collisions(rows: Sequence[Candidate]) -> dict[str, Any]:
    ...
    return {"collision_group_count": len(groups), "groups": [{"feature_sha256": digest, "owners": owners_list} for digest, owners_list in groups]}
...
def _analyze_records(records: Sequence[Candidate], ingests: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    pairs = [ingest.get("anchor_policy") for ingest in ingests]
    support_by_pair: dict[str, dict[str, int]] = defaultdict(lambda: {"raw": 0, "supported": 0})
    for ingest in ingests:
        pair = ingest.get("anchor_policy")
        pair_name = "::".join(pair) if isinstance(pair, (tuple, list)) and len(pair) == 2 else "UNKNOWN"
        support_by_pair[pair_name]["raw"] += int(ingest.get("raw_label_count", 0))
        support_by_pair[pair_name]["supported"] += int(ingest.get("supported_label_count", 0))
    support_rates = {pair: values["supported"] / values["raw"] if values["raw"] else 0.0 for pair, values in support_by_pair.items()}
    ingest = {
        "raw_label_count": sum(item.get("raw_label_count", 0) for item in ingests),
        "supported_label_count": sum(item.get("supported_label_count", 0) for item in ingests),
        "join_errors": [error for item in ingests for error in item.get("join_errors", [])],
        "firewall_errors": [error for item in ingests for error in item.get("firewall_errors", [])],
        "reliability_errors": [error for item in ingests for error in item.get("reliability_errors", [])],
        "incomplete_groups": [error for item in ingests for error in item.get("incomplete_groups", [])],
        "status_counts": dict(sum((Counter(item.get("status_counts", {})) for item in ingests), Counter())),
        "duplicate_root_ids": duplicate_roots, "root_count": len(set(root_ids)), "pair_count": len(ingests),
        "metadata_consistent": len(metadata) == 1, "declared_group_count": sum(item.get("declared_group_count", 0) for item in ingests),
        "custom_feature_extractor": any(item.get("custom_feature_extractor") for item in ingests),
        "anchor_policy_pairs": pairs, "support_by_anchor_pair": support_rates,
    }
    splits = stratified_group_split(records)
    model, classes, class_contexts, fit = _fit(splits["train"])
    reports = {name: _report(splits[name], splits["train"], model, classes, class_contexts) for name in SPLITS}
    collisions = _collisions(records)
    raw_count = ingest["raw_label_count"]
    supported_count = ingest["supported_label_count"]
    gates = {
        ...
        "support_ge_0_95": supported_count / raw_count >= 0.95 if raw_count else False,
        "support_ge_0_90_each_anchor_pair": bool(support_rates) and all(rate >= 0.90 for rate in support_rates.values()),
    ...
    ceiling = all(gates[key] for key in ("support_ge_0_95", "support_ge_0_90_each_anchor_pair", "test_top3_ge_0_75", "root_bootstrap_top3_lcb_ge_0_65", "nll_improvement_ge_0_20", "unseen_le_0_10", "repeatability_top3_ge_0_90"))
    return {
        "status": "PASS_CEILING" if mechanics and ceiling else "KILLED_CEILING" if mechanics else "BLOCKED_MECHANICS",
        "features": {"public_only": list(FEATURE_NAMES), "excluded": ["IDs", "hashes", "legal_options", "opponent_request", "opponent_view", "private_state", "anchor", "family", "seat", "determinization", "search", "memory", "post_choice_state", "future_information", "WDL"]},
        "fit": fit, "classes": list(classes), "split_root_counts": {name: len({row.root for row in rows}) for name, rows in splits.items()},
        "reports": reports, "collisions": collisions, "ingest": ingest, "gates": gates,
        "target": "selection_type/context/ordering/semantic_action_path", "strength_evidence": False,
    }

def analyze_pairs(
    pairs: Sequence[tuple[str | Path, str | Path]], *, config_path: str | Path | None = None,
    trunk: Any | None = None, feature_extractor: Callable[..., Sequence[float]] | None = None,
) -> dict[str, Any]:
    ...
    if config_path is None:
        ...
        try:
            ...
            for ingest in ingests:
                if ingest.get("config_sha256") != config_sha or ingest.get("source_commit") != config.get("source_commit"):
                    config_errors.append("sidecar config/source binding differs from config bytes")
            ...
            if not configured:
                config_errors.append("config anchor policy/cell schedule is malformed")
...
def analyze(
    sidecar_path: str | Path, root_dataset_path: str | Path, *, config_path: str | Path | None = None,
    trunk: Any | None = None, feature_extractor: Callable[..., Sequence[float]] | None = None,
) -> dict[str, Any]:
