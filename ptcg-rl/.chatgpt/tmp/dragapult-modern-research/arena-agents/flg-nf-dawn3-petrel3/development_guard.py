...
def _active_candy_over_morgrem(obs,baseline):
 if _ctx(obs)!=0 or not isinstance(baseline,list) or len(baseline)!=1:return None
 options=_opts(obs);i=baseline[0]
 if not (0<=i<len(options)):return None
 option=options[i]
 if hc._cid(hc._card_at(obs,option))!=MORGREM:return None
 try:target_area=int(option.get('inPlayArea',0) or 0)
 except Exception:return None
 if target_area!=4:return None
 _,me,_=hc._players(obs);active=(me.get('active') or [None])[0]
 if hc._cid(active)!=IMPIDIMP:return None
 hand=Counter(hc._hand_ids(me))
 if hand[GRIM]<=0 or hand[CANDY]<=0:return None
 candy=_indices(obs,CANDY)
 if not candy:return None
 out=[candy[0]]
...
def _punk_up_useful_count(obs, baseline):
 ...
 need=max(need,min(4,len(baseline)))
...
def choose(obs,baseline):
 if not _legal(obs,baseline):return baseline
 for fn in (_active_candy_over_morgrem,_poffin,_stretcher,_pokepad,_punk_up_useful_count):
  try:a=fn(obs,baseline)
  except Exception:a=None
  if a is not None and _legal(obs,a):return a
 return baseline
