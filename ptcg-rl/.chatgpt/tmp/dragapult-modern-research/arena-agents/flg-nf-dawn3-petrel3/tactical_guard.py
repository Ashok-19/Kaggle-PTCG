...
_PENDING_BOSS_ATTACK=False

def reset():
    global _PENDING_BOSS_SERIAL,_PENDING_BOSS_ATTACK
    _PENDING_BOSS_SERIAL=None
    _PENDING_BOSS_ATTACK=False
...
def choose(obs,baseline):
    ...
    # A Boss target selected by this guard is already a visible Shadow Bullet
    # knockout. Make the proof sequence executable rather than relying on the
    # rest of the policy to eventually return to the attack after development.
    if _PENDING_BOSS_ATTACK:
        _PENDING_BOSS_ATTACK=False
        if context==0:
            for i,option in enumerate(options):
                semantic=pf.semantic(obs,option)
                if int(semantic.get('type',-1))==13 and int(semantic.get('attack_id',0) or 0)==937:
                    action=[i]
    ...
    # Exact midgame Boss conversion.  Do not spend the Supporter when the
    # current Active is already removable this turn by all publicly available
    # Adrena-Brain counters plus Shadow Bullet.  Otherwise a visible <=180 HP
    # bench target is a guaranteed extra prize route with the same attack.
    own_board=[]
    for card in ((me or {}).get('active') or [])+((me or {}).get('bench') or []):
        if isinstance(card,dict): own_board.append(card)
    powered_munk=sum(1 for card in own_board if hc._cid(card)==112 and hc._energy(card)>=1)
    movable_counters=min(sum(hc._damage(card) for card in own_board)//10,3*powered_munk)
    active_can_fall=False
    if opposing_active is not None and hc._cid(opposing_active) not in BLOCKING_ACTIVE:
        active_can_fall=hc._hp(opposing_active)<=180+10*movable_counters
    if active_can_fall:
        return baseline
    ...
    for card in (opponent or {}).get('bench') or []:
        ...
        # Preserve final-prize Boss authority for any prize value. Midgame Boss
        # use is limited to visible multi-prize KOs; these are the replay-proven
        # conversions and avoid speculative one-prize tempo trades.
        if prize_value<2 and prize_value<remaining:
            continue
