...
CONFIG_PATH = PTCG_RL / "configs/bc_dragapult_archetype_v3.json"
VOLUME_NAME = "kptcg-training"
SECRET_NAME = "kptcg-kaggle"
AUTH_BLOB_ENV = "KPTCG_AUTH_BLOB"
REMOTE_CORPUS_ROOT = Path("/data/corpora/bc-dragapult-archetype-v3")
...
image = (
    ...
    .add_local_file(
        CONFIG_PATH,
        remote_path="/workspace/ptcg-rl/configs/bc_dragapult_archetype_v3.json",
    )
)

app = modal.App("kptcg-bc-dragapult-archetype-v3", image=image)
...
def _load_config() -> dict[str, Any]:
    ...
    if payload.get("winner_only_labels") is not False:
        raise RuntimeError("production Dragapult archetype v3 corpus must retain all qualified teacher outcomes")
    if payload.get("archetype_wide") is not True or int(payload.get("required_archetype_card_id", -1)) != 121:
        raise RuntimeError("production Dragapult archetype v3 config is not archetype-wide")
    return payload
...
def _write_execution_report(
    *,
    ...
    report = {
        "schema_version": 1,
        "record_id": "bc-dragapult-archetype-v3-modal-execution",
        "status": "PASS",
        "volume": VOLUME_NAME,
...
def build(force: bool = False) -> dict[str, Any]:
    started = time.perf_counter()
    config = _load_config()
    bundle = REMOTE_CORPUS_ROOT / "bc-dragapult-archetype-v3.zip"
    ...
    try:
        ...
        if config["winner_only_labels"]:
            command.append("--winner-only")
        if config["archetype_wide"]:
            command.append("--archetype-wide")
        _run_stream(command)
