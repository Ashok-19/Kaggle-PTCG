...
BUNDLE_SOURCE = Path("/data/corpora/bc-dragapult-hq-v2/bc-dragapult-hq-v2.zip")
BUNDLE_SHA256 = "1161268653eeb1e70432539e8c25d2f971d876ee0b1c365ab843130f5d3d4c3b"
LOCAL_BUNDLE = Path("/tmp/bc-dragapult-hq-v2.zip")
MATERIALIZED_DIR = Path("/data/materialized/bc-dragapult-hq-v2-featurefix-v3")
MATERIALIZED_TAR = Path("/data/materialized/bc-dragapult-hq-v2-featurefix-v3.tar")
MATERIALIZED_TAR_SHA256 = Path("/data/materialized/bc-dragapult-hq-v2-featurefix-v3.tar.sha256")
...
app = modal.App("kptcg-bc-dragapult-hq-featurefix-v3", image=image)
...
def _copy_verified_bundle() -> dict[str, Any]:
    if not BUNDLE_SOURCE.is_file():
        raise RuntimeError(f"Dragapult v2 bundle is missing: {BUNDLE_SOURCE}")
    digest = hashlib.sha256()
    byte_count = 0
    ...
    if observed != BUNDLE_SHA256:
        LOCAL_BUNDLE.unlink(missing_ok=True)
        raise RuntimeError(
            f"Dragapult v2 bundle SHA-256 differs: expected {BUNDLE_SHA256}, observed {observed}"
        )
    receipt = {
...
def materialize(force: bool = False) -> dict[str, Any]:
    ...
    try:
        command = [
            ...
            "16",
            "--record-id",
            "bc-dragapult-hq-v2-featurefix-v3",
        ]
        _run_stream(command)
        manifest = json.loads(
        ...
        if manifest.get("status") != "PASS_MATERIALIZED_BC_READY":
            raise RuntimeError("Dragapult v2 materializer did not report PASS")
        source = manifest.get("source", {})
        if source.get("test_episode_bodies_read") != 0:
            raise RuntimeError("Dragapult v2 materialization did not preserve sealed test bodies")
        archive = _ensure_tar()
        training_volume.commit()
        return {
