"""Native, public-state Frost Barrier next-turn damage capsule, v2.
...
DEFAULT_CONFIG = ROOT / "configs/deterministic/phase_a_frost_barrier_capsule_v2.json"
DEFAULT_REPORT = ROOT / "reports/deterministic/phase-a-frost-barrier-capsule-v2.json"
...
# Conservative public-log confounds for the interval between Barrier and
# the expiry response.  An unexplained event fails the capsule closed.
INTERVAL_CONFOUND_EVENT_NAMES = frozenset({
    "SWITCH", "EVOLVE", "DEVOLVE", "CHANGE", "POISONED", "BURNED",
    "ASLEEP", "PARALYZED", "CONFUSED",
})
BASE_CONFOUND_KEYS = frozenset({
    "target_still_active", "target_serial_stable", "target_card_stable",
    "target_status_free", "target_not_ko",
})
...
def load_config(path: Path = DEFAULT_CONFIG) -> dict[str, Any]:
    ...
    expected = {"schema_version", "record_id", "knowledge_base_ids", "candidate", "opponent", "formulations", "response_strata", "assets", "limits", "attack_contract", "deck_specs"}
    if set(config) != expected or config["schema_version"] != 2 or config["record_id"] != "phase-a-frost-barrier-capsule-v2":
    ...
    if [row["id"] for row in config["formulations"]] != ["same_game_expiry", "no_barrier_baseline", "same_game_expiry_1043", "no_barrier_baseline_1043"]:
    ...
    if limits["minimum_primary_expiry_pairs"] != 2 or limits["minimum_primary_expiry_pairs_per_candidate_seat"] != 1 or limits["minimum_primary_baseline_target_groups"] != 2:
        raise ValueError("primary proof thresholds must remain preregistered")
    ...
    if config["response_strata"] != ["latias_strafe_1089", "kyogre_swirling_waves_1043"]:
        raise ValueError("response strata must remain preregistered and ordered")
...
def _validate_assets(config: Mapping[str, Any]) -> dict[str, str]:
    ...
    if 1043 not in cards[721]["attack_ids"] or attacks[1043]["damage"] != 130 or attacks[1043]["energy_counts"] != [1, 0, 0, 2, 0, 0, 0, 0, 0, 0, 0, 0]:
        raise ValueError("secondary Kyogre attack does not match the local card table")
    ...
    strafe = next((row for row in rows if row["Card ID"] == "754" and row["Move Name"] == "Strafe"), None)
    waves = next((row for row in rows if row["Card ID"] == "721" and row["Move Name"] == "Swirling Waves"), None)
    abomasnow = next((row for row in rows if row["Card ID"] == "723" and row["Move Name"] == "Frost Barrier"), None)
    if strafe is None or strafe["Cost"] != "●" or strafe["Damage"] != "40" or strafe["Weakness"] != "n/a" or strafe["Resistance (Type)"] != "n/a":
        raise ValueError("card-data Strafe static/no-confound contract changed")
    if waves is None or waves["Type"] != "{W}" or waves["Cost"] != "{W}{W}●" or waves["Damage"] != "130":
        raise ValueError("card-data Swirling Waves static/cost contract changed")
    if abomasnow is None or abomasnow["Weakness"] != "{M}" or abomasnow["Resistance (Type)"] != "n/a":
        raise ValueError("card-data Frost Barrier target weakness/resistance contract changed")
    return observed
...
def _option_score(observation: EngineObservationV1, request: SelectionRequestV1, index: int, *, fixed_attack: bool, response_attack_id: int = 1089) -> tuple[int, int, int]:
    ...
    if option.option_type == 14:
        score = -1000
    if fixed_attack and option.attack_id == response_attack_id:
        score = 10000
...
def _choose_scored(observation: EngineObservationV1, request: SelectionRequestV1, *, fixed_attack: bool, response_attack_id: int = 1089) -> Any:
    builder = CompoundActionBuilder(request)
    candidates = [index for index, option in enumerate(request.options) if option.available]
    candidates.sort(key=lambda index: _option_score(observation, request, index, fixed_attack=fixed_attack, response_attack_id=response_attack_id), reverse=True)
    if request.selection_type == 0 and request.min_count == 0:
        candidates = [index for index in candidates if _option_score(observation, request, index, fixed_attack=fixed_attack, response_attack_id=response_attack_id)[0] > 0][:1]
...
class FixedOpponentPolicy:
    ...
    def __init__(self, response_attack_id: int = 1089) -> None:
        self.response_attack_id = response_attack_id
    ...
    def choose(self, observation: EngineObservationV1, request: SelectionRequestV1) -> Any:
        return _choose_scored(observation, request, fixed_attack=True, response_attack_id=self.response_attack_id)
...
def _resolve_pending_attack(item: Mapping[str, Any], after: EngineObservationV1) -> dict[str, Any] | None:
    ...
    if old is None or new is None or not isinstance(changes[0].fields.get("value"), int):
        return None
    attack_fields = item["attack_event"].get("fields", {})
    if (
        attack_fields.get("attackId") != item["attack_id"]
        or attack_fields.get("playerIndex") != item["player"]
        or attack_fields.get("cardId") != item["attacker_card_id"]
        or attack_fields.get("serial") != item["attacker_serial"]
    ):
        return None
    hp_fields = changes[0].fields
    if (
        hp_fields.get("cardId") != old.card_id
        or hp_fields.get("playerIndex") != item["target_player"]
        or hp_fields.get("serial") != target_serial
    ):
        return None
    value = int(changes[0].fields["value"])
    if value >= 0 or old.hp is None or new.hp is None or old.hp - new.hp != -value:
        return None
    if old.damage is None or new.damage is None or new.damage - old.damage != -value:
        return None
    return {
...
def _confounds_clear(confounds: Mapping[str, Any], *, require_interval: bool = False) -> bool:
    if not isinstance(confounds, Mapping) or not BASE_CONFOUND_KEYS.issubset(confounds):
        return False
    if require_interval and "no_interval_confounds" not in confounds:
        return False
    return all(bool(value) for value in confounds.values())
...
def _run_game(config: Mapping[str, Any], formulation: str, game_index: int, state: dict[str, Any]) -> dict[str, Any]:
    ...
    secondary = formulation.endswith("_1043")
    response_attack_id = 1043 if secondary else int(config["attack_contract"]["fixed_attack_id"])
    response_card_id = 721 if secondary else int(config["attack_contract"]["fixed_card_id"])
    opponent = expand_deck(config["deck_specs"]["opponent_1043" if secondary else "opponent"])
    candidate_player = game_index % 2
    decks = (candidate, opponent) if candidate_player == 0 else (opponent, candidate)
    candidate_policy = BarrierPolicy(game_index % 3) if formulation.startswith("same_game_expiry") else NoAttackPolicy(None, game_index % 3)
    opponent_policy = FixedOpponentPolicy(response_attack_id)
    ...
    captured_event_transitions: set[int] = set()
    response_target_serial: int | None = None
    response_source_serial: int | None = None
    barrier_proof_complete = False

    def capture_interval_events(events: tuple[Any, ...], transition_id: int) -> None:
        if barrier_record is None or transition_id in captured_event_transitions:
            return
        captured_event_transitions.add(transition_id)
        interval_events.extend(
            {"transition": transition_id, "event": _event_dict(event)}
            for event in events
        )

    def process_resolved(record: dict[str, Any]) -> None:
        nonlocal barrier_record, response_target_serial, response_source_serial, barrier_proof_complete
        before = record.pop("before")
        record["game_index"] = game_index
        record["candidate_player"] = candidate_player
        if record["attack_id"] == 1047 and formulation.startswith("same_game_expiry") and barrier_record is None:
            ...
            if (
                target is None or target.card_id != response_card_id or attacker is None or attacker.card_id != 723
                or record["acting_player"] != candidate_player
                or attacker.attached_energy_count < 3 or record["hp_delta"] != 200
                or not _confounds_clear(record["confounds"])
            ):
            ...
            return
        if record["attack_id"] != response_attack_id:
            return
        if formulation.startswith("same_game_expiry"):
            ...
            if (
                record["acting_player"] != 1 - candidate_player
                or record["attacker_card_id"] != response_card_id
                or record["target_card_id"] != 723
            ...
            if expected_window:
                ...
                response_source_serial = record["attacker_serial"]
            elif record["target_serial"] != response_target_serial or record["attacker_serial"] != response_source_serial:
                state["invariant_failures"]["response_serial_changed_before_expiry"] += 1
                return
            record["response_source_serial"] = response_source_serial
            record["attacker_weakness_type"] = -1
            record["attacker_resistance_type"] = -1
            record["interval_event_names"] = sorted({
                item["event"].get("event_name")
                for item in interval_events
                if item["event"].get("event_name") in INTERVAL_CONFOUND_EVENT_NAMES
            })
            record["interval_confound_events"] = [
                item for item in interval_events
                if item["event"].get("event_name") in INTERVAL_CONFOUND_EVENT_NAMES
            ]
            record["confounds"]["no_interval_confounds"] = not record["interval_confound_events"]
            state["responses"].append(record)
            if not expected_window:
                for prior in state["responses"]:
                    if prior.get("game_index") == game_index and prior.get("barrier_window") is True:
                        prior["interval_event_names"] = list(record["interval_event_names"])
                        prior["interval_confound_events"] = list(record["interval_confound_events"])
                        prior["confounds"]["no_interval_confounds"] = record["confounds"]["no_interval_confounds"]
                if record["interval_confound_events"]:
                    state["invariant_failures"]["interval_confound_event"] += len(record["interval_confound_events"])
                barrier_proof_complete = True
            return
        record["target_required_card_id"] = 723
        if record["acting_player"] == 1 - candidate_player and record["attacker_card_id"] == response_card_id and record["target_card_id"] == 723 and _confounds_clear(record["confounds"]):
            record["attacker_weakness_type"] = -1
            record["attacker_resistance_type"] = -1
            state["responses"].append(record)
    ...
    try:
        while transition < config["limits"]["request_cap_per_game"]:
            ...
            events = after.public_events
            if barrier_record is not None and not barrier_proof_complete:
                protected = _entity(
                    after,
            ...
            selected_attack_ids = [request.options[index].attack_id for index in action.submitted_original_indices if request.options[index].attack_id in {1047, response_attack_id}]
            for attack_id in selected_attack_ids:
                ...
                if any(event.event_name == "HP_CHANGE" and event.fields.get("playerIndex") == item["target_player"] and event.fields.get("serial") == item["target_serial"] for event in events):
                    ...
                    pending.append(item)
            capture_interval_events(events, transition)
            raw = raw_after
            transition += 1
        else:
...
def _verdict(formulation: str, state: Mapping[str, Any]) -> tuple[str, str]:
    ...
    secondary = formulation.endswith("_1043")
    response_attack_id = 1043 if secondary else 1089
    response_card_id = 721 if secondary else 754
    base_damage = 130 if secondary else 40
    protected_damage = base_damage - 30
    if formulation.startswith("same_game_expiry"):
        proof_games = set()
        for barrier in state["barriers"]:
            game = barrier["game_index"]
            covered = [row for row in rows if row.get("game_index") == game and row.get("attack_id") == response_attack_id and row.get("attacker_card_id") == response_card_id and row.get("barrier_window") is True and row.get("hp_delta") == protected_damage and _confounds_clear(row.get("confounds", {}), require_interval=True)]
            expired = [row for row in rows if row.get("game_index") == game and row.get("attack_id") == response_attack_id and row.get("attacker_card_id") == response_card_id and row.get("barrier_window") is False and row.get("hp_delta") == base_damage and _confounds_clear(row.get("confounds", {}), require_interval=True)]
            if covered and expired and covered[0]["target_serial"] == expired[0]["target_serial"] and covered[0].get("protected_source_serial") == expired[0].get("protected_source_serial") and covered[0].get("response_source_serial") == expired[0].get("response_source_serial"):
                proof_games.add(game)
        proof_seats = {next((row.get("candidate_player") for row in state.get("games", ()) if row.get("game_index") == game), None) for game in proof_games}
        minimum_pairs = int(state.get("minimum_primary_expiry_pairs", 2))
        minimum_per_seat = int(state.get("minimum_primary_expiry_pairs_per_candidate_seat", 1))
        per_seat = Counter(next((row.get("candidate_player") for row in state.get("games", ()) if row.get("game_index") == game), None) for game in proof_games)
        threshold_ok = len(proof_games) >= minimum_pairs and all(per_seat.get(seat, 0) >= minimum_per_seat for seat in (0, 1))
        if threshold_ok and not state["invariant_failures"] and not secondary:
            return "PASS", "card 754 attack 1089 dealt 10 on the immediately next opponent turn and 40 after expiration to the same card-723 source serial"
        reason = f"captured barriers={len(state['barriers'])}, complete same-game expiry pairs={len(proof_games)}, proof_seats={sorted(seat for seat in proof_seats if seat is not None)}"
        if secondary:
            reason += "; 1043 is retained as a diagnostic stratum only because the native barrier sequence was sparse; card-data Water-vs-Metal weakness/resistance confounds are excluded"
        else:
            reason += "; controls remain incomplete"
        return "PARTIAL", reason
    by_target: dict[tuple[int, int, int, int], list[dict[str, Any]]] = {}
    for row in rows:
        if _confounds_clear(row.get("confounds", {})) and row["attack_id"] == response_attack_id and row["attacker_card_id"] == response_card_id and row["hp_delta"] == base_damage:
            by_target.setdefault((row["game_index"], row["target_serial"], row["attacker_serial"], row["acting_player"]), []).append(row)
    qualifying = [group for group in by_target.values() if len(group) >= 2]
    minimum_groups = int(state.get("minimum_primary_baseline_target_groups", 2))
    if len(qualifying) >= minimum_groups and not state["invariant_failures"] and not secondary:
        return "PASS", "same fixed attack measured at 40 damage twice without Barrier"
    if secondary:
        return "PARTIAL", f"captured {sum(len(group) for group in qualifying)} candidate 1043 baseline groups; native barrier sequence was sparse, while card-data weakness/resistance confounds are excluded"
...
def run_experiment(config: Mapping[str, Any], config_path: Path = DEFAULT_CONFIG) -> dict[str, Any]:
    ...
    for formulation in config["formulations"]:
        ...
        state = _new_state()
        state["minimum_primary_expiry_pairs"] = config["limits"]["minimum_primary_expiry_pairs"]
        state["minimum_primary_expiry_pairs_per_candidate_seat"] = config["limits"]["minimum_primary_expiry_pairs_per_candidate_seat"]
        state["minimum_primary_baseline_target_groups"] = config["limits"]["minimum_primary_baseline_target_groups"]
    ...
    response_contract_audit = {
        "primary_required_response_attack_id": 1089,
        "primary_required_response_card_id": 754,
        "primary_required_response_static_damage": 40,
        "secondary_diagnostic_response_attack_id": 1043,
        "secondary_diagnostic_response_card_id": 721,
        "secondary_diagnostic_response_static_damage": 130,
        ...
        "card_table_primary_response_damage": attacks.get(1089, {}).get("damage"),
        "card_table_secondary_response_damage": attacks.get(1043, {}).get("damage"),
        "observed_attack_ids": observed_attack_ids,
        "primary_exact_response_available": 1089 in observed_attack_ids and attacks.get(1089, {}).get("damage") == 40,
        "secondary_attempted": 1043 in observed_attack_ids,
        "secondary_weakness_excluded": True,
        "secondary_note": "Card data binds 1043 to a Water attack while card 723 has Metal weakness and no resistance; the secondary stratum is not promoted only because its native barrier sequence was sparse.",
    }
    return {
        "schema_version": 2, "record_id": config["record_id"], "created_at_utc": datetime.now(timezone.utc).isoformat(),
