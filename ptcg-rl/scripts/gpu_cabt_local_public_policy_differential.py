...
def _gpuabi(repo_root: Path, decks: np.ndarray) -> tuple[int, int, np.ndarray, np.ndarray]:
    include = repo_root / "src/ptcg_rl/gpu_cabt/native"
...
def main() -> None:
    ...
    decks = np.stack(
        (
            _deck(repo_root / ".chatgpt/tmp/today-lucario-variants/lucario-modern-v1/deck.csv"),
            _deck(repo_root / ".chatgpt/tmp/aura-upgrade/arena-agents/dragapult-ex/deck.csv"),
        )
    )
