...
OUTPUT_ROOT = Path("/data/runs/bc-dragapult-final-v7-live-rehearsal/3.7m")
MODEL_LABEL = "3.7m"
BATCH_SIZE = 16
SEQUENCE_LENGTH = 32
MAX_TARGETS_PER_STEP = 512.0
EXACT_ANCHOR_TOLERANCE = 0.0015
LIVE_ALL_TOLERANCE = 0.001
...
app = modal.App("kptcg-bc-dragapult-live-v7-rehearsal", image=image)
...
def run(source_commit: str, force: bool = False) -> dict[str, Any]:
    ...
    report_path = OUTPUT_ROOT / "live-v7-rehearsal-report.json"
    if OUTPUT_ROOT.exists():
        ...
        if not force:
            raise RuntimeError(f"partial v7 rehearsal output exists: {OUTPUT_ROOT}")
    ...
    if not sweep.torch.cuda.is_available():
        raise RuntimeError("v7 rehearsal requires CUDA")
    if not sweep.torch.cuda.is_bf16_supported():
        raise RuntimeError("v7 rehearsal requires BF16 support")
    ...
    exact_train_1150 = subset(exact_train_records, exact_train, 1150.0)
    rehearsal_all = [*live_train, *exact_train]
    rehearsal_high = [*live_train_1090, *exact_train_1150]
    ...
    stages: list[dict[str, Any]] = [
        {
            "name": "stage-r1-live-plus-exact-all",
            "episodes": rehearsal_all,
            "learning_rate": 5.0e-6,
            "epochs": 2,
            "selection_view": "live_all",
        },
        {
            "name": "stage-r2-high-plus-exact-1150",
            "episodes": rehearsal_high,
    ...
    for stage_index, stage in enumerate(stages):
        ...
        if targets_per_step > MAX_TARGETS_PER_STEP:
            raise RuntimeError("v7 rehearsal update density exceeded the 512-target guard")
        ...
        stage_start_live_all = _metric(best_views["live_all"])
        ...
        for epoch in range(1, int(stage["epochs"]) + 1):
            ...
            eligible = (
                ...
                and live_all_nll <= stage_start_live_all + LIVE_ALL_TOLERANCE
            )
            ...
            if improved:
                ...
                receipt = save_training_checkpoint(
                    ...
                    league={
                        "kind": "dragapult-live-v7-rehearsal",
    ...
    final_receipt = save_training_checkpoint(
        ...
        league={
            "kind": "dragapult-live-v7-rehearsal-final-selection",
            "source_commit": source_commit,
            "v5_initializer_sha256": V5_CHECKPOINT_SHA256,
            "selected_source_checkpoint_path": str(best_checkpoint_path),
            "selected_source_checkpoint_sha256": best_checkpoint_sha256,
            "quality_weighting": "rehearsal curriculum: live 1050+ mixed with exact-all, then live 1090+ mixed with exact-1150",
        },
    ...
    report: dict[str, Any] = {
        "schema_version": 1,
        "record_id": "bc-dragapult-live-v7-rehearsal",
        "status": "PASS_BC_LIVE_V7_REHEARSAL",
        "source_commit": source_commit,
        "device": str(device),
        ...
        "corpora": {
            ...
            "exact_train_all": len(exact_train),
            "exact_train_1150": len(exact_train_1150),
            "rehearsal_all": len(rehearsal_all),
            "rehearsal_high": len(rehearsal_high),
        ...
        "selection_contract": {
            ...
            "maximum_stage_relative_live_all_regression": LIVE_ALL_TOLERANCE,
    ...
    print(
        json.dumps(
            {
                "event": "live_v7_rehearsal_complete",
