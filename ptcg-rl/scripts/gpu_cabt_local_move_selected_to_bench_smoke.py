...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Official CPU vs CUDA selected Hand-to-Bench movement"
    )
...
def _official_snapshots(
    ...
    source = f"""#include <iostream>\n#include "All.h"\nint main(){{\n InitializeAll();GameConfig c={{}};c.seed=1;c.recordLog=false;c.deviceRand=false;const int d0[60]={{{ids0}}};const int d1[60]={{{ids1}}};for(int i=0;i<60;++i){{c.decks[0].cards[i]=d0[i];c.decks[1].cards[i]=d1[i];}}\n int first_player,target,count;while(std::cin>>first_player>>target>>count){{BattleData b;b.init(c);State&s=b.state;for(int p=0;p<2;++p)for(int i=0;i<60;++i){{int ref;std::cin>>ref;s.players[p].deck[i]=CardRef(ref);}}s.firstPlayer=first_player;s.changed=true;for(int p:s.basicPlayerOrder())Draw(s,p,FIRST_HAND);auto&ps=s.players[target];for(int i=0;i<ps.hand.size();++i)s.getCard(ps.hand[i]).cardId=i<6?{setup_card_id}:{filler_card_id};SetupBenchPokemon(s,target);for(int i=0;i<count;++i){{int x;std::cin>>x;s.selected.push_back(x);}}s.setSelectedCardTarget();MoveToBenchSelected(s,target);auto emit=[](long long v){{std::cout<<v<<' ';}};emit(target);emit((int)ps.hand.size());emit((int)ps.bench.size());emit((int)s.targetList.size());emit(0);emit(s.moveCounter);emit((int)s.changed);for(int i=0;i<7;++i)emit(i<ps.hand.size()?(int)ps.hand[i].cardIndex:-1);for(int i=0;i<8;++i)emit(i<ps.bench.size()?(int)ps.bench[i].cardIndex:-1);for(int i=0;i<8;++i){{if(i<(int)s.targetList.size()){{const auto&t=s.targetList[i];const auto&card=s.getCard(t.card);emit((int)t.card.cardIndex);emit(t.moveCounter);emit(card.cardId);emit(card.moveCounter);emit(card.attachMoveCounter);emit(card.skillOrder);emit(card.damage);emit((int)card.playerIndex);emit((int)card.area);emit((int)card.preArea);emit((int)card.reverse);emit((int)card.abilityUsed.size());emit(card.turnState[1]);}}else for(int f=0;f<13;++f)emit(-1);}}emit((int)s.firstPlayer);std::cout<<'\\n';}}}}\n"""
    lines: list[str] = []
    ...
    with tempfile.TemporaryDirectory(prefix="gpu-cabt-move-bench-") as tmp:
...
def main() -> int:
    ...
    names = (
        ...
        "gpu_cabt_move_selected_to_bench",
        "gpu_cabt_move_selected_to_bench_snapshot",
    )
    ...
    module.get_function("gpu_cabt_move_selected_to_bench")(
        (blocks,), (threads,), (states, runtimes, targets_d, np.int32(args.env_count))
    )
    module.get_function("gpu_cabt_move_selected_to_bench_snapshot")(
    ...
    bench_counts_by_pattern = {
        str(pattern): sorted(
            set(actual[np.arange(args.env_count) % len(patterns) == index, 2].tolist())
        )
        for index, pattern in enumerate(patterns)
    }
    counts_valid = all(
        bench_counts_by_pattern[str(pattern)] == [len(pattern)] for pattern in patterns
    )
    appear_values = actual[:, 34::13]
    moved_appear_valid = True
    for env_index in range(args.env_count):
        count = len(patterns[env_index % len(patterns)])
        if count and not np.all(appear_values[env_index, :count] == (1 << 24)):
            moved_appear_valid = False
            break
        ...
        json.dumps(
            {
                ...
                "bench_counts_by_pattern": bench_counts_by_pattern,
                "bench_counts_valid": counts_valid,
                "moved_appear_valid": moved_appear_valid,
    ...
    return 0 if differential_match and errors_clear and counts_valid and moved_appear_valid else 1
