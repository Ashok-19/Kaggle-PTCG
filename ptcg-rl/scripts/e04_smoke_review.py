...
class E04SmokeReviewError(ValueError):
    pass
...
def load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise E04SmokeReviewError(f"cannot load JSON {path}: {error}") from error
    if not isinstance(value, dict):
        raise E04SmokeReviewError(f"JSON root must be an object: {path}")
    return value
...
def relative(path: Path) -> str:
    try:
        ...
        raise E04SmokeReviewError(f"evidence path escapes project root: {path}") from error
...
def review_smoke(
    *,
    ...
    if authorization.stage != "smoke" or authorization.games != 10:
        raise E04SmokeReviewError("authorization is not the exact ten-game smoke stage")
    ...
    if consumed.authorized:
        raise E04SmokeReviewError("ten-game smoke request remains authorized")
    if consumed_request.get("authorization_scope") != "CONSUMED_AFTER_TEN_GAME_APPROVED_EXECUTION":
        raise E04SmokeReviewError("ten-game smoke authorization is not consumed")
    ...
    if consumed_request.get("authorization_snapshot_sha256") != authorization_sha256:
        raise E04SmokeReviewError("consumed authorization snapshot hash differs")
    if consumed.output_directory != authorization.output_directory:
        raise E04SmokeReviewError("authorization output directory changed after execution")
    ...
    if sha256_file(runner_path) != consumed_request.get("runner_sha256"):
        raise E04SmokeReviewError("smoke runner hash differs")
    if sha256_file(prerequisite_path) != consumed_request.get(
        "prerequisite_evidence_sha256"
    ):
        raise E04SmokeReviewError("single-trace prerequisite evidence hash differs")
    ...
    if private_report.get("status") != "PASS" or private_report.get(
        ...
        raise E04SmokeReviewError("native smoke report did not pass")
    execution = private_report.get("execution")
    if not isinstance(execution, Mapping):
        raise E04SmokeReviewError("native smoke execution record is invalid")
    if (
        ...
        raise E04SmokeReviewError("native smoke execution boundary differs")

    report_authorization = private_report.get("authorization")
    if not isinstance(report_authorization, Mapping):
        raise E04SmokeReviewError("native smoke report authorization is invalid")
    if (
        report_authorization.get("sha256") != authorization_sha256
        or report_authorization.get("stage") != "smoke"
        or report_authorization.get("games") != 10
        ...
        raise E04SmokeReviewError("native smoke report authorization differs")
    ...
    if not isinstance(report_games, list) or not isinstance(ledger_games, list):
        raise E04SmokeReviewError("native smoke game ledger is invalid")
    if report_games != ledger_games or len(report_games) != 10:
        raise E04SmokeReviewError("native smoke report and game ledger differ")
    if private_report.get("qualification") != qualification:
        raise E04SmokeReviewError("native smoke qualification summary differs")
    ...
    if private_report.get("bridge_checkpoint", {}).get("sha256") != bridge_sha256:
        raise E04SmokeReviewError("native smoke bridge hash differs")
    if private_report.get("game_ledger", {}).get("sha256") != games_sha256:
        raise E04SmokeReviewError("native smoke game-ledger hash differs")

    episodes = bridge_state.get("episodes")
    if not isinstance(episodes, Mapping) or len(episodes) != 10:
        raise E04SmokeReviewError("native smoke bridge episode count differs")

    per_game: list[dict[str, Any]] = []
    terminal_result_counts = {"-1": 0, "0": 0, "1": 0}
    for expected_index, record in enumerate(report_games):
        if not isinstance(record, Mapping):
            raise E04SmokeReviewError("native smoke game record is invalid")
        episode_id = record.get("episode_id")
        if record.get("game_index") != expected_index or not isinstance(
            episode_id, str
        ):
            raise E04SmokeReviewError("native smoke game ordering differs")
        if record.get("failure") is not None:
            raise E04SmokeReviewError(f"native smoke game failed: {episode_id}")
        summary = record.get("summary")
        episode = episodes.get(episode_id)
        if not isinstance(summary, Mapping) or not isinstance(episode, Mapping):
            raise E04SmokeReviewError("native smoke game evidence is incomplete")
        if episode.get("closed") is not True or episode.get("boundary") != "TERMINAL":
            raise E04SmokeReviewError(f"native smoke episode did not close: {episode_id}")
        ...
        if not isinstance(boundaries, Mapping) or set(boundaries) != {"0", "1"}:
            raise E04SmokeReviewError(f"native smoke boundaries differ: {episode_id}")
        if not isinstance(owners, Mapping) or set(owners) != {"0", "1"}:
            raise E04SmokeReviewError(f"native smoke owners differ: {episode_id}")
        if any(
            boundary.get("terminal") is not True
            or boundary.get("truncation") is not False
            for boundary in boundaries.values()
        ):
            raise E04SmokeReviewError(f"native smoke terminal boundary differs: {episode_id}")
        ...
        if terminal_result not in (-1, 0, 1) or terminal_result != episode.get(
            ...
            raise E04SmokeReviewError(f"native smoke terminal result differs: {episode_id}")
        if (
            ...
            raise E04SmokeReviewError(f"native smoke game counters differ: {episode_id}")
        if maximum_error > REPLAY_TOLERANCE:
            raise E04SmokeReviewError(f"native smoke replay tolerance failed: {episode_id}")
    ...
    if any(qualification["reliability"].values()):
        raise E04SmokeReviewError("native smoke zero-tolerance counters are nonzero")
    if qualification["maximum_compound_log_probability_absolute_error"] > REPLAY_TOLERANCE:
        raise E04SmokeReviewError("native smoke aggregate replay tolerance failed")

    public_report: dict[str, Any] = {
        "schema_version": 1,
        "record_id": "e04-ten-game-smoke-v1",
        "created_at_utc": private_report.get("created_at_utc"),
        "source_path": relative(public_report_path),
        "producer": "scripts/e04_smoke_review.py",
        "status": "SUCCEEDED",
        "decision": "PASS",
        "stage": "smoke",
        "authorization": {
            ...
            "hundred_game_stage_authorized": False,
        },
        "assets": {
...
def main() -> int:
    ...
    output_directory = ROOT / "private/g3/e04/ten-game-smoke-v1"
    parser.add_argument(
    ...
    parser.add_argument(
        "--consumed-request",
        type=Path,
        default=ROOT / "configs/e04_ten_game_smoke_request_v1.json",
    )
    parser.add_argument(
        "--games",
        type=Path,
