...
from ptcg_rl.g1.semantic import EVENT_FEATURES, OPTION_FEATURES, encode_numeric, semantic_snapshot
...
def snapshot(**kwargs):
    return semantic_snapshot(raw_observation(**kwargs), "battle", 0, CARD_HASH)
...
def test_multiselect_min_max_stop_and_uniqueness() -> None:
    ...
    _, request = snapshot(options=options, min_count=2, max_count=3)
    assert request is not None
...
def test_permutation_round_trips_original_engine_indices() -> None:
    _, request = snapshot(options=[{"type": 0, "number": i} for i in range(5)])
    assert request is not None
...
def test_all_official_selection_context_and_option_codes_are_structurally_preserved() -> None:
    options = [
        ...
        {"type": 5, "area": 4, "index": 0, "playerIndex": 0, "energyIndex": 0},
        ...
        {"type": 13, "attackId": 7},
        {"type": 14},
        {"type": 15, "cardId": 0, "serial": 0},
        {"type": 16, "specialConditionType": 4},
    ]
    raw = raw_observation(options=options, min_count=1, max_count=1)
    for selection_type in range(11):
        raw["select"]["type"] = selection_type
        raw["select"]["context"] = selection_type
        _, request = semantic_snapshot(raw, "battle", selection_type, CARD_HASH)
        assert request is not None
        assert request.selection_type == selection_type
        assert [option.option_type for option in request.options] == list(range(17))
...
def test_more_than_64_options_are_preserved_and_overflow_fails() -> None:
    _, request = snapshot(options=[{"type": 0, "number": i} for i in range(70)])
    observation, _ = snapshot(options=[{"type": 0, "number": i} for i in range(70)])
...
def test_semantic_and_tensor_json_round_trip_preserves_missing_vs_zero() -> None:
    ...
    assert "serial" not in EVENT_FEATURES
    builder = CompoundActionBuilder(request)
    builder.choose(0)
    action = builder.build()
