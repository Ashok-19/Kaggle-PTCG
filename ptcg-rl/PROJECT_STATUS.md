...
Last completed milestone: exact private Kaggle CPU G3a cloud correctness plan frozen and independently reviewed  
Current gate: G3a recurrent PPO correctness proof  
Gate status: G2 PASS / R1 PASS / G3a BLOCKED / NOT_REVIEWED  
Next review required before: publishing the prepared private G3a assets, launching the bounded PPO correctness run, additional episode JSON transfer, Modal execution, deck freeze, or submission
...
| G3a PPO correctness smoke | blocked | `configs/g3a_evaluation_v1.json`, `configs/g3a_local_correctness_v1.json`, `configs/kaggle/g3a_cloud_correctness_v1.json`, `reports/artifacts/g3a-ppo-local-correctness-v1.json`, `reports/artifacts/g3a-ppo-local-correctness-review-v1.json`, `reports/artifacts/g3a-cloud-correctness-plan-v1.json`, `reports/artifacts/g3a-cloud-correctness-plan-review-v1.json`, `reports/gates/g3a.json` | Contract, local harness and exact private Kaggle CPU plan passed independent review; explicit launch approval, manual stable-asset publication and cloud qualification remain open |
...
1. Present the exact immutable private Kaggle CPU G3a correctness plan for one explicit approve-or-reject decision; do not publish assets or launch automatically.
2. On approval only, manually publish or update the stable private dataset and single notebook to frozen numeric version `1`, attach the exact dataset, select CPU with internet/GPU/TPU off, set the explicit authorization variable and run without editing.
3. After completion, list all saved outputs with pagination, download the output manifest first, verify every listed byte count and SHA-256, run the strict independent review and keep G3a blocked on any mismatch.
4. Keep additional replay retrieval, action-supervision training, Modal execution, deck freeze and submission blocked until their separate approval gates.
