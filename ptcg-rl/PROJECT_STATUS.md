...
Last completed milestone: greedy G3a cloud rollout bug reproduced and corrected; private dataset version 3 published and byte-verified  
...
| G3a PPO correctness smoke | blocked | `configs/g3a_evaluation_v1.json`, `configs/g3a_local_correctness_v1.json`, `configs/kaggle/g3a_cloud_correctness_v1.json`, `reports/artifacts/g3a-ppo-local-correctness-v1.json`, `reports/artifacts/g3a-ppo-local-correctness-review-v1.json`, `reports/artifacts/g3a-cloud-correctness-plan-v1.json`, `reports/artifacts/g3a-cloud-correctness-plan-review-v1.json`, `reports/jobs/g3a-cloud-input-publication-v3.json`, `reports/gates/g3a.json` | Contract, local harness, corrected exact plan, explicit user approval and private dataset version 3 passed review after seeded-rollout correction; the user-run notebook outputs and strict run review remain open |
...
No active long-running jobs. G1R contract repair, one-million-operation corpus, final-source parity, 10,080-game arena, throughput matrix, six-hour RSS soak, independent raw-artifact review, the G2 10,000-game T4 x2 neural reliability qualification, the bounded G3a local PPO correctness matrix, the complete G3a cloud-plan freeze/review lifecycle and private input-dataset publication are complete. Dataset `ashok205/kptcg-g3a-correctness-inputs` version `3` is `READY` and independently byte-verified; versions `1` and `2` are historical. The first complete user-run version-2 notebook failed strict final review after about one hour because cloud rollouts were greedy; no passing G3a cloud result exists. Verified project compute cost remains USD `0`.
