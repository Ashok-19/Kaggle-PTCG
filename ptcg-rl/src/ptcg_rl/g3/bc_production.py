...
TRAINING_RECORD_ID = "e01-production-recurrent-bc-request-v1"
        ...
        selected.append(
            {
                ...
                "bytes": int(item["bytes"]),
                "sha256": _require_sha(item.get("sha256"), "training replay SHA-256"),
...
def validate_training_request(root: Path, request_path: Path) -> ProductionAssets:
    ...
    if request.get("status") != "READY_UNAUTHORIZED" or request.get("request_ready") is not True:
        raise ProductionBCContractError("production BC request is not ready and unauthorized")
    authorization = request.get("authorization")
    if not isinstance(authorization, Mapping) or any(value is not False for value in authorization.values()):
        raise ProductionBCContractError("production BC request contains an authorization")
    manifest_path, manifest = _load_manifest(root, request)
    records = training_records(manifest)
    corpus = request.get("corpus", {})
    if corpus.get("record_listing_sha256") != canonical_listing_hash(records):
        raise ProductionBCContractError("training record listing hash differs")
    ...
    publication = request.get("publication_dependency", {})
    publication_path = root / _safe_relative(publication.get("request_path"), "publication request path")
    if sha256_file(publication_path) != _require_sha(publication.get("request_sha256"), "publication request SHA-256"):
        raise ProductionBCContractError("publication dependency hash differs")
    validate_publication_request(root, publication_path)
    for name in ("initial_checkpoint", "card_data"):
        ...
        raw = path.read_bytes()
        if len(raw) != _positive_int(asset.get("bytes"), f"{name} bytes"):
            raise ProductionBCContractError(f"{name} byte count differs")
        if sha256_bytes(raw) != _require_sha(asset.get("sha256"), f"{name} SHA-256"):
            raise ProductionBCContractError(f"{name} SHA-256 differs")
    _verify_runner(root, request)
    return ProductionAssets(request=request, request_path=request_path, manifest=manifest, manifest_path=manifest_path)
    ...
    with tempfile.TemporaryDirectory(prefix="kptcg-production-bc-") as temporary:
        ...
        for item in records:
            ...
            if len(raw) != int(item["bytes"]) or sha256_bytes(raw) != item["sha256"]:
                raise ProductionBCContractError(f"mounted replay differs for episode {item['episode_id']}")
...
def execute_authorized_training(
    ...
    runner_path = root / str(assets.request["runner"]["path"])
    approval = _approval(approval_path, request_path, runner_path, "E01_PRODUCTION_RECURRENT_BC_APPROVAL_V1")
    authorization = approval.get("authorization", {})
    for field in ("replay_body_read", "optimizer_steps", "training", "private_kaggle_cpu"):
        if authorization.get(field) is not True:
            raise ProductionBCContractError(f"training approval does not authorize {field}")
    if any(authorization.get(field) is not False for field in ("gpu", "tpu", "internet", "label_materialization", "model_promotion", "submission", "git_commit", "git_push")):
        raise ProductionBCContractError("training approval expands forbidden scope")
    if output_dir.exists():
    ...
    schedule = build_balanced_schedule(
        chunks, seed=int(execution["seed"]), epochs=int(execution["maximum_epochs"])
    )
    total_steps = sum(len(epoch) for epoch in schedule)
    if total_steps > int(execution["maximum_optimizer_steps"]):
        raise ProductionBCContractError("derived optimizer schedule exceeds approved cap")
    ...
    for epoch_index, epoch_steps in enumerate(schedule, start=1):
        ...
        receipt = save_training_checkpoint(
            ...
            league={"kind": "production-recurrent-bc-candidate", "production_eligible": False},
    ...
    candidate_eligible = int(best["epoch"]) > 0 and float(best["validation_mean_nll"]) < baseline_nll - 1e-6
    report = {
        "schema_version": 1,
        "record_id": "e01-production-recurrent-bc-execution-v1",
        "status": "PASS_CANDIDATE_READY_FOR_SEPARATE_EVALUATION" if candidate_eligible else "PASS_NO_IMPROVED_CANDIDATE",
        "request_sha256": sha256_file(request_path),
        "optimizer_steps": completed,
        "maximum_optimizer_steps": int(execution["maximum_optimizer_steps"]),
        "epochs": len(schedule),
        ...
        "candidate_checkpoint": None if int(best["epoch"]) == 0 else f"epoch-{int(best['epoch'])}.pt",
