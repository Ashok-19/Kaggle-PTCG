...
__device__ __forceinline__ void effect_targets_status(
    BattleCoreState& state, const BattleRuntimeState& runtime, const RuleTableView& rules,
    ...
    if (runtime.target_count > 0) {
        ...
        if (valid_area_ref(state,r) && state.all_card[r.card].area == kAreaActive) {
            ...
            if(kind==0)effect_burn(state,rules,p);else if(kind==1)effect_poison(state,rules,p,poison_count);else if(kind==2)effect_sleep(state,rules,p);
        }
        return;
    }
...
__device__ __noinline__ bool effect_instant_96_110(
    ...
    switch(effect.effect_type){
        ...
        case 98: for(gc_i32 p=0;p<2;++p)if(is_target_player(owner,p,effect.target.target_player))effect_poison(state,rules,p,8);return true;
        case 99: for(gc_i32 p=0;p<2;++p)if(is_target_player(owner,p,effect.target.target_player))effect_poison(state,rules,p,16);return true;
        case 100: effect_targets_status(state,runtime,rules,effect,2); return true;
        case 101: for(gc_i32 p=0;p<2;++p)if(is_target_player(owner,p,effect.target.target_player))effect_confuse(state,rules,p);return true;
        case 102: for(gc_i32 p=0;p<2;++p)if(is_target_player(owner,p,effect.target.target_player))effect_paralyze(state,rules,p);return true;
        case 103:
